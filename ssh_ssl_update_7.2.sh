...
# take /tmp as the working directory
wdir='/tmp'
old_pwd=$(pwd)
ssl_tar='openssl-1.0.1t.tar.gz'
# ssh_tar='openssh-6.7p1.tar.gz'
ssh_tar='openssh-7.2p2.tar.gz'
target_ssl='1.0.1t'
target_ssh='7.2p2'
...
update_open_ssh() {
	...
	if [ $? -ne 0 ]; then
		./configure --prefix=/usr --sysconfdir=/etc/ssh --with-pam --with-zlib --with-md5-passwords --without-hardening --without-openssl-header-check
	fi
	...
	\cp ${wdir}/openssh-7.2p2/contrib/redhat/sshd.init /etc/init.d/sshd
	chkconfig --add sshd
	service sshd start	
}
...
if [ $(cat $ssh_cfg_file | grep -c '^\ *PasswordAuthentication yes') -ge 1 ]; then
	echo "we can login with root without any modifications"
else
	#echo "let's do some modification"
